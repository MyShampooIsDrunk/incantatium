...
public class ModifiedRaid extends Raid {
    public static final SpawnLocation RAVAGER_SPAWN_LOCATION = SpawnRestriction.getLocation(EntityType.RAVAGER);
    public static final MapCodec<ModifiedRaid> CODEC = RecordCodecBuilder.mapCodec(
            instance -> instance.group(
                            ...
                            Status.CODEC.fieldOf("status").forGetter(raid -> raid.status),
                            ...
                            Difficulty.CODEC.fieldOf("difficulty").forGetter(raid -> raid.difficulty)
                    )
                    .apply(instance, ModifiedRaid::new)
    );
    public static final int MAX_BAD_OMEN_LEVEL = 10;
    ...
    private final int raidOmenLevel;
    ...
    public ModifiedRaid(BlockPos center, Difficulty difficulty, int raidOmenLevel) {
        super(center, difficulty);
        this.active = true;
        this.preRaidTicks = 300;
        this.bar.setPercent(0.0F);
        this.center = center;
        this.raidOmenLevel = raidOmenLevel;
        this.waveCount = this.getMaxWaves(difficulty, raidOmenLevel);
        this.status = Status.ONGOING;
        this.difficulty = difficulty;
    }

    private ModifiedRaid(
            boolean started,
            boolean active,
            long ticksActive,
            int raidOmenLevel,
    ...
    public boolean start(ServerPlayerEntity player) {
        ...
        if (statusEffectInstance == null) {
            ...
            //TODO: when creating a new raid include this code to get the raid omen level VVV
    ...
    public void tick(ServerWorld world) {
        if (!this.hasStopped()) {
            if (this.status == Status.ONGOING) {
                ...
                this.active = world.isChunkLoaded(this.center.asLong());
                    ...
                    this.bar
                            .setName(
                                    EVENT_TEXT.copy()
                                            ...
                                            .append(" / ")
                                            .append(this.getMaxWaves(difficulty, raidOmenLevel) + "")
    ...
    private void spawnNextWave(@NotNull ServerWorld world, BlockPos pos) {
        boolean bl = false;
    ...
    //TODO
    public int getMaxWaves(Difficulty difficulty, int omenLevel) {
